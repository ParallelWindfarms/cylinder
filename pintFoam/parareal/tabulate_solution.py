# ~\~ language=Python filename=pintFoam/parareal/tabulate_solution.py
# ~\~ begin <<lit/paranoodles.md|pintFoam/parareal/tabulate_solution.py>>[0]
...
def tabulate_np(step: Solution, y_0: Array, t: Array) -> Array:
    ...
    return y
# ~\~ end
# ~\~ begin <<lit/parareal.md|tabulate-np>>[0]
def tabulate_np(step: Solution, y_0: Array, t: Array) -> Array:
    y = np.zeros(dtype=y_0.dtype, shape=(t.size,) + y_0.shape)
    y[0] = y_0
    for i in range(1, t.size):
...
# ~\~ end
# ~\~ end
# ~\~ begin <<lit/parareal.md|pintFoam/parareal/tabulate_solution.py>>[0]
from .abstract import (Solution, Vector)
from typing import (Sequence, Any)
import numpy as np
